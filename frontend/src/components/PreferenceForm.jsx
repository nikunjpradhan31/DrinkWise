...
const PreferenceForm = ({ onCompleted }) => {
  ...
  if (loading) {
    return <p className="text-sm text-slate-300">Loading your preferences…</p>;
  }

  if (error && !Object.values(preferences).some(val => val !== null && val !== undefined)) {
    return (
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <p className="text-sm text-slate-300">
        Set your drink preferences so we can recommend drinks you'll actually enjoy.
      </p>

      <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
        {/* Sweetness Preference */}
        <div className="rounded-lg bg-slate-900/70 border border-slate-700/70 p-3">
      ...
      >
        {submitting ? "Saving your preferences…" : "Save Preferences and Continue"}
      </button>
    </form>
  );
};
